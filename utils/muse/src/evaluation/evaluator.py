...
class Evaluator(object):
    ...
    def dist_mean_cosine(self, to_log):
        ...
        for dico_method in ['nn', 'csls_knn_10']:
            ...
            to_log['mean_cosine-%s-%s-%i' % (dico_method, _params.dico_build, dico_max_size)] = mean_cosine.item()
    ...
    def eval_dis(self, to_log):
        ...
        for i in range(0, self.src_emb.num_embeddings, bs):
            emb = Variable(self.src_emb.weight[i:i + bs].data, requires_grad=False)
        ...
        for i in range(0, self.tgt_emb.num_embeddings, bs):
            emb = Variable(self.tgt_emb.weight[i:i + bs].data, requires_grad=False)
